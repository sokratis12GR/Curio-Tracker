...
class UnifiedSettingsSection:
    def __init__(self, parent, tracker, theme_manager, tree_manager: TreeManager):
        ...
        self.enable_poeladder_var = ctk.BooleanVar(value=get_setting("Application", "enable_poeladder", c.ENABLE_POELADDER))
    ...
    def build(self, frame, row_start):
        ...
        theme_cb = ctk.CTkComboBox(frame, variable=self.theme_selector_var, values=c.theme_modes, width=160)
        ...
        # ---- Player League ----
        ctk.CTkLabel(frame, text="Player League:").grid(row=row, column=0, sticky="w")
        ...
        ctk.CTkCheckBox(frame, text="poeladder", variable=self.enable_poeladder_var, command=self._toggle_poeladder).grid(
            row=row, column=0, columnspan=1, sticky="w"
        )
        row += 1

        # ---- Data League ----
        ctk.CTkLabel(frame, text="Data League (poe.ninja):").grid(row=row, column=0, sticky="w")
    ...
    def _update_application_theme(self, *_):
        ...
        switch_mode(self.tree_manager, self.theme_manager, val)
...
# Helper functions
# -------------------------------
def switch_mode(tree_manager: TreeManager, theme_manager, mode=c.DEFAULT_THEME_MODE):

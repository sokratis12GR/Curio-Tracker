...
# toasts_duration_seconds = get_setting('Application', "toasts_duration_seconds",
#                                             5)  # default: 5 seconds toasts duration.
# images_visible = True
# original_image_col_width = c.IMAGE_COL_WIDTH
# TYPE_DISPLAY_MAP = {
#     "Trinket": "Trinket",
...
# style.configure("Treeview", rowheight=c.ROW_HEIGHT, padding=0)
#
...
# tree.column("#0", width=c.IMAGE_COL_WIDTH, anchor="center", stretch=False)
#
# tree_columns = [
...
#         img = img.resize((c.IMAGE_COL_WIDTH - 4, c.ROW_HEIGHT), Image.LANCZOS)
#         img = pad_image(img, left_pad=-20, top_pad=0,
#                         target_width=c.IMAGE_COL_WIDTH, target_height=c.ROW_HEIGHT)
...
#
# # ---------- Sorting ----------
# sort_reverse = {"img": False, "item": False, "value": False, "type": True, "stack_size": False, "area_level": False,
#                 "layout": False, "player": False, "league": False, "time": True, "record": True, "tier": False}
#
#
...
# time_options = [
#     "All", "Last hour", "Last 2 hours", "Last 12 hours", "Today", "Last 24 hours", "Last week", "Last 2 weeks",
...
#         tree.column("img", width=c.IMAGE_COL_WIDTH, minwidth=20, stretch=False)
...
#
#
# images_visible = get_setting("Columns", "img_visible", True)
#
#
# def toggle_images():
#     global images_visible
#     images_visible = not images_visible
#     set_setting("Columns", "img_visible", images_visible)
#
#     for iid in tree.get_children():
...
#             tree.item(iid, image='')
#
#     tree.column("#0", width=c.IMAGE_COL_WIDTH if images_visible else 0)
#
#
# def toggle_image_column(show):
#     toggle_img_btn.state(["!disabled"] if show else ["disabled"])
#     tree.column("#0", width=c.IMAGE_COL_WIDTH if show else 0)
#
#
...
#     saved_state = get_setting("Columns", col, True)
...
#     set_setting('User', 'poe_user', tracker.poe_user)
#     if c.DEBUGGING:
...
#     set_setting('User', 'poe_league', tracker.league_version)
#     if c.DEBUGGING:
#         print(f"[DEBUG] League set to: {tracker.league_version}")
#
#
# tracker.poe_user = get_setting('User', 'poe_user', getattr(tracker, 'poe_user', ''))
# poe_player_var.set(tracker.poe_user)
#
# tracker.league_version = get_setting('User', 'poe_league', getattr(tracker, 'poe_league', league_versions[0]))
...
# console_visible = get_setting('Application', 'console_visible', True)
#
#
# def toggle_console():
...
#     set_setting('Application', 'console_visible', console_visible)
#
#
...
#     set_setting('Application', 'are_toasts_enabled', are_toasts_enabled)
...
#
# toasts.TOASTS_DURATION = int(get_setting('Application', 'toasts_duration_seconds', 5))
...
#     set_setting('Application', 'toasts_duration_seconds', toasts.TOASTS_DURATION)
#     if c.DEBUGGING:
...
#     info_labels['capture'].config(
#         text=f"Press {curio_keybinds.get_display_hotkey('capture')} to capture all curios on screen (no duplicates).")
#     info_labels['snippet'].config(
#         text=f"Press {curio_keybinds.get_display_hotkey('snippet')} to snippet a region to capture allows duplicates.")
#     info_labels['layout'].config(
#         text=f"Press {curio_keybinds.get_display_hotkey('layout_capture')} to set current layout.")
